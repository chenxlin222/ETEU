...
%   This script runs the implementation of ETEU for visual tracking.
%   Some functions are borrowed from other papers (ARCF, BACF, STRCF UAV123 Benchmark)
%   and their copyright belongs to the paper's authors.

clear; clc; close all;
addpath(genpath('.'));

%pathAnno = 'E:\codes\datasets\UAV123_10fps\anno\UAV123_10fps\';
pathAnno = 'E:\codes\datasets\UAV123\tracker_benchmark_v1.1\trackers\ETEU\seq\';
seqs = configSeqs_demo_for_ETEU;
seqs{1}.name
...
for idxSeq=1:length(seqs)
    ...
    %   To run ARCF-HC, comment the line above and uncomment the line below
     res = run_ETEU(subS, 0, seqs{1}.name, 0);
